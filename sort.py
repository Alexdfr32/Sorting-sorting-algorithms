...
def quicksortleft(a, left, right ):
  if left< right:
    ...
    quicksortleft(a, left, piv-1)

    quicksortleft(a, piv+1, right)
...
def partitionrmid(a, left, right):
  ...
  a[left],a[pivot]=a[pivot],a[left]#we put the pivoit in the first place
...
@func_set_timeout(200)
def quicksortright(a, left, right ):
  if left< right:
...
def partitionerright(a, left, right):
  pivot=right#choosing right as pivot
  #pivot=middle
...
def partitionright(a, left, right):
  pivot=left 
  #print("pivot is ",a[left],"   ")
